...
pub fn add_program(program_id: &Pubkey, accounts: &[AccountInfo], name: String) -> ProgramResult {
    ...
    for ind in 0..name_storage_numeration + 1 {
        ...
        if config.validator_numeration % constants::MAX_PROGRAMS_PER_NAME_STORAGE_ACCOUNT == 0
            && current_name_storage_account.owner == &system_program::id()
        {
            ...
            invoke_signed(
                ...
                &[
                    ...
                    current_name_storage_account.clone(),
                ],
                &[&[
                    b"name_storage",
                    &ind.to_be_bytes(),
            ...
            name_storage_data
                .serialize(&mut &mut current_name_storage_account.data.borrow_mut()[..])?;
        }
        ...
        if ind == name_storage_numeration {
            ...
            let transfer_lamports: i128 = Rent::get()?.minimum_balance(name_storage.get_space())
                as i128
                - current_name_storage_account.lamports() as i128;
            if transfer_lamports > 0 {
                ...
                current_name_storage_account
                    .realloc(name_storage.get_space(), true)
    ...
    let transfer_lamports: i128 = Rent::get()?.minimum_balance(storage_data.get_space()) as i128
